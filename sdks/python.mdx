...
Define evals with `evaluate(...)`, then run them with `promptlayer eval run <path>`. Start from the [Quickstart](/sdks/evals/quickstart) or the [SDK Evals overview](/sdks/evals/overview).
