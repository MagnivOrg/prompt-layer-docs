...
<Card
  title="Python SDK"
  icon="github"
  href="https://github.com/MagnivOrg/prompt-layer-library?tab=readme-ov-file"
>
...
Define evals with `evaluate(...)`, then run them with `promptlayer eval run <path>`.

Start with the [Quickstart](/sdks/evals/quickstart), then see [SDK Evals overview](/sdks/evals/overview).

## Table scorecards

Use `client.tables.sheets.scorecards` to configure criteria, run calculations, and inspect row-level results.
...
rows = client.tables.sheets.scorecards.list_rows(
    table_id,
    sheet_id,
    {"calculation_id": run["calculation_id"], "verdict": "fail", "limit": 50},
)
...
if rows["rows"]:
    ...
    print(row["aggregate_verdict"], row["step_results"])
```

Use these methods:

- `get(table_id, sheet_id)`
- `configure_scorecard(table_id, sheet_id, body)`
- `delete(table_id, sheet_id)`
- `migrate_legacy_score(table_id, sheet_id, body)`
- `recalculate(table_id, sheet_id, body=None)`
- `cancel(table_id, sheet_id, body=None)`
- `get_calculation(table_id, sheet_id, calculation_id)`
- `list_rows(table_id, sheet_id, options=None)`
- `get_row(table_id, sheet_id, row_index, options=None)`

See the full API contract in the [table scorecard REST reference](/reference/table-sheet-scorecard-get). The REST API uses `PATCH` to configure scorecards, returns `202 Accepted` when recalculation is queued, paginates rows with `cursor` / `next_cursor`, and returns row breakdown fields at the top level.
