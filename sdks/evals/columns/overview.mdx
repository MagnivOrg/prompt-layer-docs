---
title: "Columns"
description: "Processing columns for SDK evaluate(...) — column(title, ColumnType, config)."
sidebarTitle: "Columns"
icon: "table-columns"
---

Processing columns transform row data before scorers run.

**Source of truth for column type behavior and configs:** [Column Types](/features/tables/column-types) (and [Columns](/features/tables/columns) for the Tables UI).

This page covers the **SDK API** — `column(title, ColumnType.<TYPE>, config)`. Differences vs the dashboard:

- There are no typed `*_column` helpers; use `column(...)` with `ColumnType`
- Do not declare processing columns as `TEXT` — `Input` / `Expected` / `Output` are built-ins
- For scorecard checks, use [typed scorer helpers](/sdks/evals/scorers/overview) instead of scorer column types here
- Config keys generally match Tables; Python helpers use snake_case field names in `config` where the backend expects them
