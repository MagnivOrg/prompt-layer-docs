...
Processing columns transform row data before scorers run. Build them with `column(title, ColumnType.<TYPE>, config)`.

For scorecard checks, use [typed scorer helpers](/sdks/evals/scorers/overview). Full product catalog: [Tables column types](/features/tables/column-types).
...
You do not create `ColumnType.TRACE` with `column(...)`. When a scorer references Trace (Trajectory's default is `"Trace"`), the SDK scaffolds a reserved `Trace` column and imports each case's span tree.

Trajectory scores spans named `Tool: <name>`. See [Runner and tracing](/sdks/evals/agent-tracing).

Reserved titles: `Trace`, `Trace.price`, `Trace.latency`, `Trace link`, `total_trace_time`, `total_price`, `Expected Trace`.
