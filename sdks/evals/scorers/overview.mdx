...
Scorers are scorecard steps on the experiment sheet. Prefer the typed helpers below. You can also pass a named function (custom code) or `column(title, type, config)`.

Reference columns by **title** (`"Output"`, `"Expected"`, `"Trace"`, or a processing column). Built-ins: `Input`, `Expected`, `Output`, and when needed `Trace` / `Expected Trace`.

## Weight, Required, and Thresholds

Every typed helper accepts weight, required, and thresholds. Scorecards aggregate with a weighted mean (defaults: pass `0.8`, warn `0.6`).
...
Compare two columns. By default that is `Output` vs `Expected` as strings. Pass exactly two column titles in `sources`.
