...
Compare two columns. By default that is `Output` vs `Expected` as strings. Pass exactly two column titles in `sources`. Matching rules match the Compare type in [Column Types](/features/tables/column-types).
...
Scores `Tool: <name>` spans from the imported `Trace` against accepted scenarios. Any `runner` works if those spans exist — emit them with a framework helper or `traceTool` as described in [Runner and tracing](/sdks/evals/agent-tracing), or copy a harness from the [Quickstart](/sdks/evals/quickstart).
