...
For Trajectory, tools must appear as `Tool: <name>` spans on the imported Trace; use a framework helper or [`traceTool`](/features/observability/traces/manual-tracing#trace-tools). The runner and tool-span rules live on [Runner](/sdks/evals/agent-tracing).
