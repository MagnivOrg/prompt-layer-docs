...
The `runner` is any callable that takes in an `input` and returns a final value. The stack tabs below are **examples**, not an allowlist — see [Runner and tracing](/sdks/evals/agent-tracing).
...
- Dataset cases use `input`, plus `expected` (expected output) and/or `expected_trace` / `expectedTrace` (Trajectory config) when scorers need them.
...
Pick a matching example, or use `traceTool` for a custom agent.
