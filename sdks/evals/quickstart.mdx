...
The `runner` is any callable that takes in an `input` and returns a final value. The stack tabs below are **examples**, not an allowlist — any harness works as long as you follow [Building an eval](/sdks/evals/building-an-eval) and [Runner](/sdks/evals/agent-tracing).
...
Call `instrument_openai_agents` / `instrumentOpenAIAgents` once, then run your Agent from the eval `runner`. Tool spans are collected automatically — no `traceTool` needed. For non-eval app wiring, use the [OpenAI Agents SDK](/features/integrations#openai-agents-sdk) integration.
...
Call `get_claude_config` / `getClaudeConfig` **inside** the runner so the Claude session nests under the eval span. Pass `plugin` and `env` into `ClaudeAgentOptions`. For non-eval app wiring, use the [Claude Code](/features/integrations#claude-code) integration.
...
Enable `experimental_telemetry` on the AI SDK call. `evaluate(...)` already registers PromptLayer's OpenTelemetry exporter, so tool spans nest under the eval case — no separate `NodeSDK` setup for the eval path. Outside evals, register OTEL with the [Vercel AI SDK](/features/integrations#vercel-ai-sdk) integration.
...
`evaluate(...)` also always enables tracing on the client it creates, so tool spans nest under each eval case. More on callbacks in [LiteLLM](/features/integrations#litellm) and the [LiteLLM PromptLayer docs](https://docs.litellm.ai/docs/observability/promptlayer_integration).
  </Tab>
...
For non-eval app wiring, use the [Pydantic AI](/features/integrations#pydantic-ai) integration.
  </Tab>
...
Install and configure the plugin with the [OpenClaw](/features/integrations#openclaw) integration.
  </Tab>
</Tabs>
