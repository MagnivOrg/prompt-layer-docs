...
The `runner` is any callable that takes in an `input` and returns a final value. The stack tabs below are **examples**, not an allowlist — see [Building an eval](/sdks/evals/building-an-eval) and [Runner](/sdks/evals/agent-tracing).
