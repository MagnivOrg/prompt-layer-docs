...
The `runner` is any callable that takes an `input` and returns a final value. The tabs below are copy-paste examples, not an allowlist — any harness works if it follows [Building an eval](/sdks/evals/building-an-eval) and [Runner](/sdks/evals/agent-tracing).
...
Call `instrument_openai_agents` / `instrumentOpenAIAgents` once, then run your Agent from the eval `runner`. Tool spans are collected automatically — no `traceTool` needed. The same helper works outside evals via the [OpenAI Agents SDK](/features/integrations#openai-agents-sdk) integration.
...
Enable PromptLayer tracing with `PromptLayer(enable_tracing=True)`, wrap tools with `traceTool` for Trajectory, and set LiteLLM's PromptLayer callback for request logging. LiteLLM does not emit separate `Tool:` spans on its own — `traceTool` is a no-op unless that `PromptLayer` instance has tracing enabled.
