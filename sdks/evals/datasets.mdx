...
Part of [Building an eval](/sdks/evals/building-an-eval). Every case needs `input`.

- `expected` — expected **output** (for Contains, Compare, and similar)
- `expected_trace` / `expectedTrace` — Trajectory **config** (accepted tool scenarios), not the expected answer text
...
| `expected_trace` / `expectedTrace` | `Expected Trace` | Trajectory config (which tools should run) |
