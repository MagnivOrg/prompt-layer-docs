...
Every case needs `input`. Add `expected` for Compare-style scorers, and `expected_trace` / `expectedTrace` for Trajectory.
...
| Field | Becomes |
| --- | --- |
| `input` | `Input` (passed to `runner`) |
| `expected` | `Expected` |
| `expected_trace` / `expectedTrace` | `Expected Trace` |
