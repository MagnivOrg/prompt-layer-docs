...
Use SDK evals when your agent needs to run on your own infrastructure. For example access to internal data, actual tool execution code, and custom harnesses.

This also makes evals a natural fit for CI/CD: run `promptlayer eval run` in your pipeline to catch trajectory or output regressions before they ship, while every result still lands on a PromptLayer Table experiment sheet for you to inspect.
