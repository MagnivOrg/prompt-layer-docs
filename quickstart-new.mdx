---
title: "Quickstart new"
description: "Create, run, and evaluate your first PromptLayer prompt."
icon: "rocket"
---

PromptLayer helps you manage prompts outside your code, run them with your LLM provider, and evaluate changes before they reach production. In this quickstart, you will create a cake recipe prompt, run it in the dashboard and from code, then build an evaluation for it.

## Prerequisites

Before you start, make sure you have:

- A [PromptLayer account](https://dashboard.promptlayer.com/create-account)
- A PromptLayer API key from your workspace
- A provider API key for the model you plan to use, such as `OPENAI_API_KEY`
- Python or JavaScript installed locally

<Info>
  PromptLayer runs LLM requests from your environment. Your provider API keys are
  not sent to PromptLayer servers.
</Info>
...
Open **Logs** in PromptLayer and search for `cake-recipe` or filter by the `quickstart` tag. The log should show the input variables, generated output, model, latency, tags, and metadata.
...
To compare how your prompt performs across GPT, Claude, and Gemini, create a new evaluation for model comparison.
...
This helps you find the best price and performance balance for your use case. PromptLayer supports major providers, plus any OpenAI-compatible API via [custom base URLs](/features/custom-providers).
...
- [Prompt editor and versioning](/features/prompt-registry/prompt-editor-versioning) - Edit prompts, review diffs, and track versions
- [Backtest prompt changes](/onboarding-guides/backtesting-prompt-changes) - Test new versions against historical requests
- [Evaluations](/features/evaluations/overview) - Learn how evaluation pipelines work
- [Python SDK](/sdks/python) - Use PromptLayer from Python
- [JavaScript SDK](/sdks/javascript) - Use PromptLayer from JavaScript or TypeScript
