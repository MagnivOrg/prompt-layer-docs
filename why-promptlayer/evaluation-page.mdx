...
The Evaluations page allows users to run a prompt against a Dataset, a batch of test cases built from a JSON or CSV file. This documentation will guide you through the various features and options available on the Evaluations page.
...
The Evaluations page let users create new datasets or select from the previous used ones to run their prompts against. A name should be assigned to each dataset, which will enable searching by name later. A dataset can be created from a JSON or CSV file. These files should contain the test cases that will be used as input variables for the prompt template. Here's an explanation of each format:
