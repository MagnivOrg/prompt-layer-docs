...
The Evaluations page lets users execute a prompt against a dataset, which is built from test cases in either a JSON or CSV file. This documentation will guide you through the various features and options available on the Evaluations page.
...
On the Evaluations page, users can either create new datasets or choose from previously used ones to test their prompts. Each dataset can be assigned a name for easy search later on. Datasets can be created from JSON or CSV files, which includes the test cases serving as input variables for the prompt template. Below is a breakdown of each format:
