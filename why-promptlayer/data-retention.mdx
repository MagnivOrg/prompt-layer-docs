...
<Warning color="yellow">
  Reducing the retention period may permanently delete request data older than
  the new period. This action cannot be undone.
</Warning>
...
- **Change deliberately**: Because shortening the period can permanently delete older data, confirm the new period is correct before applying it.
...
<AccordionGroup>
  ...
  <Accordion title="Does changing the policy delete data I've already logged?">
    Retention is applied to each record when it is logged, based on the workspace's period at that time. Changing the policy only affects **newly ingested** data — records already logged keep the retention they were assigned. The one exception is reducing the period: data logged *after* the change that then exceeds the shorter window will age out and be permanently deleted.
  </Accordion>
