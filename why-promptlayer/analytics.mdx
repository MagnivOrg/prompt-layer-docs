...
If you want a quick read on request usage, open Wrangler in **Prompt usage analysis** and ask directly (for example: "Create a graph of most used prompts over the past week"). Wrangler can probe request analytics first, confirm which prompts have traffic, and then build a chart view from that data.
