...
Evaluating your prompts is crucial to ensuring they perform as intended before deployment. For example, an **ai-poet** prompt generating haikus can be evaluated for creativity, syllable accuracy, and engagement. Similarly, an [**ai-doctor-agent**](/onboarding-guides/agentic-workflows) providing diagnostic suggestions can be assessed to determine which input parameters produce the most accurate and cost-effective results. Evaluation process allows you to optimize performance and drive data-informed improvements.
...
2. Click the **Create Dataset** button.
   1. If you have a .csv file, upload it or (Optional) download example datasets provided by PromptLayer. (<a href="./example-dataset/haiku.csv" download>Download Example Dataset</a>)
