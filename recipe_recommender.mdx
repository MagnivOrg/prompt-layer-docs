...
The "Recipe-Recommender" is your digital chef's assistant, helping you find the perfect recipe based on what you're in
the mood for. Think of it as a search engine, but instead of searching the entire web, it dives deep into a collection
of delicious recipes to suggest the best match for you.
...
First let's create a `settings.py` file to store our application level constants.
<CodeGroup>
```python settings.py
...
Next we will create a `openai_utils.py` file. This includes a `get_vector_for_text` function that will allow us to pass
in text and will respond with a vector. We will later store these vectors in Pinecone.
...
After this, create a new file called `io_utils.py`. Here we will create a helper function called `get_recipe_filepaths`
that will retrieve a list of recipe file paths.
...
Next, create a `files.py` file that will contain the functionality used to parse file names and read text files. The
file name (without the extension) will be used as the unique ID for each recipe vector that we store in Pinecone.
...
Create a `pinecone_populate.py` file next. This file will first create a Pinecone index. Once created, it will read
each of our recipes, generate a vector from it, and lastly insert it as a new record in our Pinecone index.
...
Before we query our index, let's first create a `suggestion.py` file. This logic will allow us to create a prompt which
is text that includes:
1. user input indicating food they are interested in (eg. "I like chicken")
...
Lastly, we will create a `pinecone_query.py` file. In this file we will provide a text snippet of food that we are
interested in. Using that text, we will query our Pinecone index to find the nearest vector matches (closely related
recipe IDs). Using those IDs, we can retrieve the original recipe text from our data. With these, we can generate our
final prompt which will be sent to OpenAI, using the `suggest_final_recipe` function created in the previous step.
...
{food}`, we'll try using the text `My favorite food is {food}` instead.
