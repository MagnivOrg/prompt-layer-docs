...
**Blank Smart Table Sheet Creation**
Create empty sheets via the public API and build them incrementally.
- Omit `source` on `POST /api/public/v2/tables/{table_id}/sheets` to create a blank sheet synchronously (`201`)
- Add columns and rows with the existing column and row endpoints after creation
- File and request-log imports still return `202` and run asynchronously
