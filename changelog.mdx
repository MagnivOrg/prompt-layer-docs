...
#### Improvements
- `Structured Search` now accepts Ctrl+Enter in addition to Cmd+Enter as a keyboard shortcut to apply filters
- Fixed an issue where `FOR_LOOP` column variable mappings were not preserved correctly when configuring dependencies
- Improved layout behavior of multi-value badges in `Structured Search` filters to better handle overflow and text wrapping
...
**Gemini 3.8 Flash Model Support**
Added support for Google's Gemini 3.8 Flash model across Google AI Studio, Vertex AI, and OpenRouter providers. This model offers enhanced reasoning capabilities with 1M+ token context length and competitive pricing.
- Available with configurable reasoning effort levels (low, medium, high)
- Supports multimodal inputs including text, images, video, audio, and PDF files
- Optimized for complex workflows and autonomous agent tasks

#### Improvements

- Updated pricing for OpenAI GPT-5.6 Sol and Sol Pro models to reflect new rates
- Enhanced `Playground` fullscreen view to properly display video content and playback controls
- Fixed duplicate tool response display issue when importing conversations into `Playground`
...
**Trace Input/Output Previews**
The trace detail view now displays a preview of the trace's initial input and final output at the top level, making it easier to understand what a trace accomplished at a glance.
- Automatically extracted from the first user message and final assistant response
- Visible in the trace summary section above the span tree
- Particularly useful for multi-step traces with tool calls

**Enhanced Span Details Panel**
The span details panel has been redesigned with a tabbed interface for easier navigation of span information.
- New tabbed layout separates metadata, inputs/outputs, and entity references
- Improved visualization of complex nested data structures
- Hover cards show quick previews of span details without opening the full panel

#### Improvements

- Added support for Gemini 3.7 Flash model with updated pricing and reasoning capabilities
- Trace entities (prompts, workflows, evaluations) are now highlighted in the trace view for better context
- Improved filtering controls for history import exclusions
- Better handling of mock tool responses in playground chat sessions
...
Real-time progress indicators now display when running evaluations via the SDK, showing row completion counts and current status as data populates.
- Dashboard automatically updates with "X of Y rows complete" while evaluations are running
- SDK can now publish planned row counts before writing data
- Explicit lifecycle states (running, completed, aborted) prevent stale progress displays
- WebSocket delivery ensures immediate UI updates without polling delays

**Enhanced Scorecard Status Reporting**
Scorecard progress now includes granular per-row status breakdowns beyond simple completion percentages.
- New verdict-based counters show passed, failed, warned, running, and queued rows separately
- Progress calculations now use database truth instead of Redis approximations for accuracy
- Partial scores remain visible during long-running evaluations

#### Improvements

- Sheet metadata updates via API no longer require title or index changes
- Grid row counts stay synchronized during optimistic row creation
- Safari layout rendering improved for version selector tooltips
- Sheet creation events now stream to connected clients for immediate list updates
...
#### Improvements
- Workflow canvas now supports spacebar panning for easier navigation when designing complex workflows
- Smart table cell content dialog resets view mode when switching between cells for more consistent behavior
- Smart table now correctly displays execution references for large truncated cell values
...
Connect and manage external MCP servers centrally in your workspace to provide tools across all prompts. Access the new MCP Servers page in workspace settings to configure servers, test connections, and discover available tools. ([docs](https://docs.promptlayer.com))

- Configure MCP servers once per workspace and reference them in multiple prompt templates
- Automatically discover and import tools from connected MCP servers
- Test server connectivity and view available tools before deployment
- Manage server credentials securely using workspace environment variables

#### Improvements

- Improved video content handling in OpenRouter request logs for better media playback
- Enhanced MCP tool execution with support for structured content responses
- Added validation to prevent MCP server deletion when referenced by active prompts
- Streamlined MCP server connection with automatic fallback between transport protocols
...
**Request and Trace Column Imports**
Smart Tables now support importing Request logs and Traces directly into anchor columns, preserving all nested metadata and execution details.
- Click "Import" in any Request or Trace column cell to link existing request logs or traces
- Imported data automatically populates nested metadata columns (latency, cost, model, tokens)
- View imported source IDs to track which requests/traces are already in your table

**Column Conversion to Anchors**
Convert existing Prompt Template columns to Request columns or Workflow columns to Trace columns while preserving cell relationships.
- Right-click column header and select conversion option
- All downstream dependent columns automatically recalculate with new anchor data
- Column titles auto-format to maintain consistency across anchor columns

#### Improvements

- Duplicate columns now preserve all nested metadata and virtual field configurations
- Smart Tables action bar limits selection display to prevent performance issues with large datasets
- Column group headers support duplicate and import operations on nested execution metadata columns
- Improved column title allocation when duplicating anchor columns ensures unique naming
- Cell content dialogs display anchor import status and allow re-import operations
...
**OpenRouter Audio Model Support**
The `Playground` now supports OpenRouter's audio-capable chat models (e.g., GPT-4 Audio) with integrated voice output capabilities.
- Select from pre-configured voice options directly in the model parameters panel
- Audio responses stream in real-time with browser-playable playback controls
- Voice parameter is automatically configured with sensible defaults for supported models

**OpenRouter Image Generation in Playground**
Image generation models from OpenRouter (including vector-capable models like Recraft) are now available in the `Playground`.
- Configure aspect ratio, quality, and output format parameters for each model
- Vector models (SVG output) automatically show appropriate format options
- Image outputs display inline with download and preview capabilities

**OpenRouter Speech Model Integration**
Text-to-speech models from OpenRouter providers (including MiniMax Speech, Deepgram, and MAI Voice) can now be used in the `Playground`.
- Model-specific voice selection with pre-populated options
- Response format automatically adapts based on provider requirements (MP3/PCM)
- Audio speed control where supported by the underlying model

#### Improvements

- `Smart Table` column paste operation now handles multiple cells in a single batch update with conflict detection
- `Smart Table` sheet navigator scrolling improved when working with tables containing many sheets
- Audio playback controls enhanced with better transport controls and seeking capabilities
- Model parameter controls now intelligently show/hide based on actual API support per model
- `Playground` streaming toggle automatically locks when required or disallowed by selected model
...
- Create multi-criterion scorecards with structured evaluation steps including STRUCTURE and ASSERT_VALID validators
...
- Support for AND/OR filter groups with span-specific conditions (SPAN_AND/SPAN_OR)
