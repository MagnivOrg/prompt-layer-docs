...
Now that we have user input referencing their food interest ("chicken") and two related recipes, we can craft a prompt from these to feed into OpenAI. Our goal is to obtain a single final recipe to recommend to the user.

To accomplish this, we will first create a prompt template using PromptLayer. When it comes to crafting prompts, prompt templates offer many benefits including ease of reuse, built-in versioning, improved organization, collaboration support, and more.

The video below will show you how to create a prompt template using the UI. We will structure our template to accept two variables, one for the users food preference and the other for the recipes.

**INSERT VIDEO HERE**

Once created, we can then retrieve our prompt template from PromptLayer.
...
To use our template, we will create a variables dictionary containing the values we want to pass in. With these values, we'll construct the full prompt and pass it to GPT-3.5-turbo to obtain a recipe suggestion.
