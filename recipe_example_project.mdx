...
With our environment ready, our initial task is to produce embeddings (vector representations) for each recipe, leveraging OpenAI's embedding API. But before diving into that, there are a few preparatory steps to handle.
...
The final goal of this project is to obtain a single final recipe to recommend to the user. To accomplish this, we will first create a prompt template using PromptLayer. A prompt template is a customizable prompt strings with placeholders for variables. When it comes to crafting prompts, prompt templates provide many benefits including reusability, built-in versioning, enhanced collaboration, and much more.
...
To use our template, we'll create a `variables` dictionary containing the values we want to pass in. Using these values, we'll construct the full prompt then pass it to GPT-3.5-turbo to obtain a recipe suggestion.
...
Lastly, we will associate the generated response with this specific prompt template in PromptLayer for tracking.
...
After running this script you will be able to see the final recipe suggestion in PromptLayer!
...
PromptLayer provides useful tools for prompt engineering. By centralizing prompt templates in the Prompt Registry, it makes it easy to track different versions of a prompt. The testing interface allows quickly trying variations. And the analytics provide visibility into how well different prompts are performing.
...
In the PromptLayer dashboard, you can view the log of all requests made. For each request, there is a score field that defaults to 0. You can manually update this to any value from 0 to 100 based on your assessment of the response quality.
...
Scoring requests provides helpful analytics over time on how your prompts are performing. It enables sorting and filtering requests to surface the most effective prompts.
...
Metadata is helpful for advanced analysis and debugging of your prompt performance. You can quickly lookup requests associated with a specific user or error case. The analytics tabs also allow filtering and segmentation based on metadata values. Metadata can also provide additional context for each recommendation request, making it easier to debug issues or anomalies. For instance, if certain users consistently receive low-scoring recommendations, the associated metadata can help identify potential causes.
