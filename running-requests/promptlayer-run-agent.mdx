...
## Alternative: Async Execution with Webhooks

Instead of polling for results, you can provide a `callback_url` when initiating the agent run. This is ideal for long-running agents and integrations that support webhooks (like Zapier's callback URL pattern).
...
This approach eliminates the need for polling and handles timeouts gracefully, making it perfect for Zapier integrations and other webhook-based systems. 