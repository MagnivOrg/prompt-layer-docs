...
The `type` field distinguishes between custom function tools (`"function"`) and built-in tools (`"web_search"`, `"file_search"`, `"code_interpreter"`, `"image_generation"`, `"google_maps"`). Built-in tools are configured in the Prompt Registry UI — see [Tool Calling](/features/prompt-registry/tool-calling#openai-and-azure-openai-responses-api) for details.
