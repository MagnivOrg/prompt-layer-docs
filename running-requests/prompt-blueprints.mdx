...
In addition to static tool definitions, you can use **tool variables** to inject tools dynamically at runtime. A tool variable acts as a placeholder in the `tools` array that gets replaced with actual tool definitions from `input_variables`.
...
At runtime, pass the tool definitions through `input_variables`:
...
The tool variable placeholders are expanded before the prompt is sent to the LLM, so the model sees a flat list of regular tool definitions. For more details, see [Tool Variables](/features/prompt-registry/tool-calling#tool-variables).
