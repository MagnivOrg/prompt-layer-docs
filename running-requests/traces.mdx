...
Use `traceTool` for tool handlers in a custom agent that does not already have a dedicated [Telemetry Integration](/features/integrations). PromptLayer names the span `Tool: <name>` and marks it as a tool call.
