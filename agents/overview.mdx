...
When you automate Tables through the REST API or PromptLayer MCP server, read [Agent and Programmatic Usage](/features/tables/agent-and-programmatic-usage) first. Table creates through MCP are hard to undo, computed column recalculation incurs provider costs, and your workspace must have provider API keys configured before queueing large runs.

The PromptLayer MCP server does not yet expose Table create, update, or delete tools. For cross-workspace Table migration, use the [Tables REST API](/reference/introduction#tables) in a script and use MCP for dependency lookup. See [Copying and Migrating Tables](/features/tables/copying-and-migrating) for the full prod-to-local and cross-workspace workflow.
...
<CardGroup cols={2}>
  <Card title="Agent and Programmatic Usage" icon="terminal" href="/features/tables/agent-and-programmatic-usage">
