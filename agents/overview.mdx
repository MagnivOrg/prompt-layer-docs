...
Smart Tables are the recommended replacement for legacy datasets and evaluations. Column configs use `{table_id}.{sheet_id}.{column_id}` resolver strings, and computed columns declare a `dependencies` array. See [Tables Data Model](/features/tables/data-model) for resolver format, dependency objects, cell shapes, and score configuration.
