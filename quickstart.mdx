...
First, make sure you have signed up for an account at www.promptlayer.com to begin 🍰
...
Pull down our prompt template with one line:

<CodeGroup>

```python Python
ai_poet_prompt = promptlayer_client.templates.get("ai-poet")
```

```js JavaScript
const ai_poet_prompt = await promptLayerClient.templates.get("ai-poet");
```

</CodeGroup>


Or specify a [release label](/features/prompt-registry/overview#release-labels) for prod/staging environments. You will need to add the release label "prod" in the PromptLayer dashboard for this line to work:
...
Add an argument to our getter to inject input variables. For example:
...
Different models expect different keyword arguments. PromptLayer can help format the prompt for all major model providers.

For this quickstart, we will format the prompt for OpenAI and then run it.
...
![Opening Log](/images/quickstart/haiku-log.png)

#### Enriching logs with metadata

PromptLayer can be used to monitor user behavior, beta deployments, and much more.

Add more [metadata](/features/prompt-history/metadata) to each log, such as the [prompt template](/features/prompt-history/tracking-templates), input variables, [tags](/features/prompt-history/tagging-requests), and [score](/features/prompt-history/scoring-requests):
...
promptlayer_client.track.metadata(request_id=pl_request_id, metadata={
  "user_id": "abc123"
})
...
const [response, pl_request_id] = await openai.chat.completions.create({
  ...ai_poet_prompt.llm_kwargs,
  model: "gpt-3.5-turbo",
  pl_tags: ["quickstart_tutorial"],
  return_pl_id: true,
});
console.log("Answer:", response.choices[0].text);

// Associate the request with the prompt template used
await promptLayerClient.track.prompt({
  request_id: pl_request_id,
  prompt_name: 'ai-poet',
...
// Add metadata to the logs
await promptLayerClient.track.metadata({
  request_id: pl_request_id,
  metadata: {
    user_id: "abc123"
  }
...
![Enriched Log Visualization](/images/quickstart/enriched-log.png)
...
Decoupling prompts from your code and [using a prompt CMS will speed up development](/why-promptlayer/prompt-cms). Read more about prompt management best practices on our [blog](https://blog.promptlayer.com/scalable-prompt-management-and-collaboration-fff28af39b9b).
