...
1. From [PromptLayer's home screen](https://www.promptlayer.com), navigate to the prompt registry ([learn more](/features/prompt-registry/overview)).
...
Next, add a user message with an input variable `{topic}`. This will be filled out by user input when interacting with the AI.
...
Remember the `{topic}` user input variable to choose the haiku topic? PromptLayer can format the f-string or Jinja2 template using this variable.
