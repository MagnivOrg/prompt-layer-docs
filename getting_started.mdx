...
const [response, pl_request_id] = openai.completions.create({
  model: "text-davinci-003",
...
```js JavaScript
const city_choice_prompt = promptlayer.prompts.get({
  prompt_name: "city_choice",
});
const city_choice_prompt_v1 = promptlayer.prompts.get({
  prompt_name: "city_choice",
  version: 1,
});
const city_choice_prompt_prod = promptlayer.prompts.get({
  prompt_name: "city_choice",
  label: "prod",
});
console.log(city_choice_prompt_v1["template"]);
```
...
const prompt_template = city_choice_prompt["template"];

const input_variables = {
  city: "Washington, D.C.",
  interests: "resorts, museums, beaches",
};
let replaced_prompt_template = prompt_template;
Object.entries(input_variables).forEach(([key, value]) => {
  replaced_prompt_template = replaced_prompt_template.replace(
    `{${key}}`,
    value
  );
});
...
const [response, pl_request_id] = await openai.completions.create({
  model: "text-davinci-003",
  prompt: replaced_prompt_template,
  pl_tags: ["getting_started_example"],
  return_pl_id: true,
});
