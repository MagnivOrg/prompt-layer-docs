...
const city_choice_prompt_v1 = await promptLayerClient.templates.get("city_choice",{ version: 1 });
const city_choice_prompt_prod = await promptLayerClient.templates.get("city_choice", { label: "prod" });
...
const city_choice_prompt = await promptlayer_client.templates.get("city_choice", {
  provider: "openai",
  input_variables,
});
