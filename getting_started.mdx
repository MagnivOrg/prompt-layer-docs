...
PromptLayer is at its core a REST library. Using our (Python SDK)[/languages/python] is equivalent to directly making requests to our API, just a little easier.
...
If you are used to working with the OpenAI SDK, all you will need to do is swap out your `import openai` for `openai = promptlayer.openai`. The rest of your code stays the same!
...
Enriching requests often requires a request_id. All PromptLayer requests have unique IDs, and these can be optionally returned when logging the request.
...
--data-raw '{
    "function_name": "openai.Completion.create",
    "kwargs": {"engine": "text-davinci-003", "prompt": "My name is", "tags": ["getting_started_example"]},
    "request_response": {...},
    "request_start_time": 1673987077.463504,
    "request_end_time": 1673987077.463504,
    "api_key": "pl_<YOUR API KEY>"
...
Users use scores in many ways. Below are some examples:
...
# Score the request
curl --location --request POST 'https://api.promptlayer.com/rest/score-request' \
--header 'Content-Type: application/json' \
--data-raw '{
...
--data-raw '{
    "function_name": "openai.Completion.create",
    "kwargs": {"engine": "text-davinci-003", "prompt": "<YOUR PROMPT>", "tags": ["getting_started_example"]},
    "request_response": {...},
    "request_start_time": 1673987077.463504,
    "request_end_time": 1673987077.463504,
    "api_key": "pl_<YOUR API KEY>"
...
curl --location --request POST 'https://api.promptlayer.com/rest/add-metadata' \
--header 'Content-Type: application/json' \
--data-raw '{
    "request_id": <YOUR REQUEST ID>,
    "metadata": {
...
The Prompt Registry handles versioning. As you can see below, we can retrieve the latest prompt or a specific stable version.
...
curl --location --request GET 'https://api.promptlayer.com/rest/get-prompt-template' \
...
Once a prompt is in the registry, we can link it to a request:
...
```bash REST
# Send a request using the PromptLayer API
curl --location --request POST 'https://api.promptlayer.com/rest/get-prompt-template' \
--header 'X-API-KEY: <YOUR PROMPTLAYER API KEY>' \
--data-raw '{
    "prompt_name": "city_choice"
}'
...
In addition to OpenAI, PromptLayer supports other models as well. 
...
```bash REST
curl --location --request POST 'https://api.promptlayer.com/rest/generate-chat-prompt' \
--header 'Content-Type: application/json' \
--data-raw '{
    "messages": [
        {"role": "system", "content": "You are a helpful AI assistant that helps people bake."},
...
### Falcon-7b & HuggingFaceHub

Here's an example of using the Falcon-7b model from HuggingFaceHub. By using LangChain with the `PromptLayerCallbackHandler`, you can access tons of LLMs.
