...
After creating a prompt template, we can retrieve it programmatically using the API. [The Prompt Registry](/features/prompt-registry) is often used as a prompt template CMS to avoid blocking prompt changes on eng rollouts.
