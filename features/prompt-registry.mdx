...
Custom metadata can be associated with individual prompt template versions. This allows you to set values such as provider, model, or temperature for each prompt template. It is important to note here that `model` attribute is reserved for the model parameters.
