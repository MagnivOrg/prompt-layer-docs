...
When fetching a prompt template, you can optionally retrieve model parameters by using `include_parameters=True`. This will provide you with a tuple containing both the prompt template and the model parameters. For example:

```python Python SDK
template, model_params = promptlayer.prompts.get("my_template", include_parameters=True)
```
...
Custom model parameters can be associated with individual prompt templates versions. This allows you to set the provider, model, temperature... etc for each prompt template.
