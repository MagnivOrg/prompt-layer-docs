...
Custom metadata can be associated with individual prompt template versions. This allows you to set values such as provider, model, temperature, or any other key-value pair for each prompt template. While the `model` attribute is reserved for the model parameters, you can add any other custom metadata as per your requirements.

<img src="/images/model-parameters.png" />

Custom non-model related metadata can be seen and edited at Template edit page.
