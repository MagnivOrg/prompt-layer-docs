...
When you fetch a prompt template, you can get the model parameters that were used by passing in the `include_parameters=True` argument. This returns a tuple with the prompt template and the model parameters. For example:

```python Python SDK
prompt, model = promptlayer.prompts.get("my_template", include_parameters=True)
```
...
You can also add model parameters to your prompt templates. This allows you to set the provider, model, temperature, max_tokens, and other parameters for each prompt template. This is useful if you want to use different parameters for different prompts.

<img src="/images/model-parameters.png" />

You can also use our python SDK to publish a prompt template with model parameters. For example:
