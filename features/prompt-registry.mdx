...
Custom metadata can be associated with individual prompt templates versions. This allows you to set the provider, model, temperature... etc for each prompt template. It is important to note here that `model` attribute is reserved for the model parameters.

<img src="/images/model-parameters.png" />

You can also use our python SDK to publish a prompt template with metadata. For example:
