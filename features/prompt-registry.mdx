...
The Prompt Registry also allows you to track groups of requests. This is particularly useful when you want to analyze the performance of a group of prompts that were used in a specific context or during a specific period of time. 

For example, you might want to track all the prompts used during a promotional campaign, or all the prompts used by a specific version of your application. By grouping these prompts together, you can analyze their performance as a whole and make informed decisions about their effectiveness.

To associate a group of requests with a specific identifier, you can use the `group_id` parameter when tracking the prompt. Here's an example:

```python Python SDK
pl_group_id - promptlayer.group.create()

promptlayer.track.group(
    request_id=pl_request_id, group_id=pl_group_id
)
```

Alternatively, use the REST API endpoint `/rest/track-group` ([read more](/reference/track-group)).

Learn more about tracking groups [here](/features/prompt-history/tracking-groups)
...
Prompt labels are a way to put a label on your prompt template to help you organize and search for them. This enables you to get a specific version of prompt using the label. You can add as many labels as you want to a prompt template with one restriction - the label must be unique across all versions. This means that you cannot have a label called `is_prod` on both version 1 and version 2 of a prompt template. This restriction is in place to prevent confusion when searching for prompt templates.

<video controls="controls">
  <source src="/videos/prompt-label.mp4" type="video/mp4" />
</video>

Later, in your code, you can use the following snippet to get the version of a prompt template with a specific label:

```python Python SDK
promptlayer.prompts.get("my_template", label="is_prod")
```

You can also use the REST API endpoint `/rest/get-prompt-template` ([read more](/reference/get-prompt-template)).

## Prompt Version Commit Messages

Prompt commit messages allow you to set a brief 72 characters-long description on each of your prompt template version to help you keep track of changes. 
...
```
They are also available in the following REST API endpoints 
