...
Once configured, PromptLayer will capture LLM calls, inputs and outputs, token usage, tool traces, workflow spans, and model metadata. Reasoning content from models that support extended thinking — including OpenAI o-series models via the Responses API — is captured from the `ai.response.reasoning` attribute and displayed as a reasoning block in the request log view.
...
Once configured, PromptLayer will capture Pydantic AI agent runs, LLM calls, tool calls, embeddings, prompts, completions, token usage, and model metadata. Thinking content from extended thinking models — for example, Anthropic's extended thinking passed through `pydantic_ai.all_messages` thinking parts — is captured and displayed in the request log view.
...
Once configured, PromptLayer will capture LangChain and LangSmith spans. Manual GenAI attributes make LLM calls, tool calls, inputs, and outputs easier for PromptLayer to render as request logs. Anthropic thinking blocks in LangChain completions (e.g., from extended thinking models like `claude-sonnet-4-6`) are captured and preserved in the request log view.
