...
Speech models take the prompt text as the input to synthesize and require a **voice** parameter. PromptLayer merges each model's advertised voices with the common OpenAI-compatible voice aliases and sorts them alphabetically. The response is returned as playable audio and logged as an audio output.
