---
title: "Legacy Scoring"
description: "Legacy sheet score configuration with score columns, Boolean and numeric scoring, custom code, and winner aggregation."
icon: "gauge"
---

<Warning>
  Legacy table scoring is deprecated for new workflows. Use [Scorecards](/features/tables/scorecards) for new evaluation workflows and migration planning.
</Warning>

Scoring turns a sheet into a summary signal you can compare across runs, prompt versions, and workflow changes.

Use score configuration when you need a single sheet-level score. Use scorecards when you need reusable criteria, row-level evidence, and calculation history.
...
| **Scorecard** | You need structured evaluation criteria, run history, and row-level evidence for regressions. | Define criteria (`steps`), aggregation, run recalculation, then inspect rows and step results. |
...
<CardGroup cols={2}>
  ...
  <Card title="Scorecards guide" icon="clipboard-check" href="/features/tables/scorecards">
