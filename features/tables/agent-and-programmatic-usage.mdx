...
<Warning>
  **Creates are hard to undo through agents.** The PromptLayer MCP server does not yet expose delete tools for Tables, sheets, or columns. Resources created through MCP or automation remain in the workspace until you remove them in the dashboard or call the REST API [Delete Table](/reference/tables-delete), [Delete Sheet](/reference/table-sheets-delete), or [Delete Column](/reference/table-sheet-columns-delete) endpoints. Treat every create, import, and column configuration as permanent until you have validated it in a non-production workspace.
</Warning>
...
When moving a workflow from legacy Evaluations or Datasets, follow [Migrate from Evaluations and Datasets](/features/tables/migrate-from-evaluations-and-datasets). To replicate a Table in another workspace, export the sheet from the dashboard, rebuild columns in the target workspace, and compare outputs before switching automation.
