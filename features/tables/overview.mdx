...
<CardGroup cols={2}>
  ...
  <Card title="Scorecards" icon="clipboard-check" href="/features/tables/scorecards">
    Define reusable criteria, run evaluations, and inspect row-level evidence.
  </Card>
...
By default, [Create a Table](/reference/tables-create) also creates `Sheet 1` with a `Column A` text column and one empty row. Pass `create_default_sheet: false` to create an empty table, then add sheets with [Create a sheet](/reference/table-sheets-create).
