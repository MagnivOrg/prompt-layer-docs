...
When the model makes tool calls, the tool call structure is flattened with a `tool_calls.` prefix. Tool names are also extracted into the `tool_names` array for easy filtering.
...
{"field": "output", "operator": "key_equals", "value": "users", "nested_key": "tool_calls.function.arguments.table"}
