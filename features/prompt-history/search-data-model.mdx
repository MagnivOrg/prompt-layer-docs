...
For chat requests, `input_text` is built by combining all messages except assistant messages. Each message is prefixed with its role in brackets and joined with double newlines:
...
In multi-turn conversations, prior assistant responses are excluded from `input_text` — only the system prompt, user messages, and other roles (e.g. `tool` results) are included. The last assistant message in the conversation is indexed separately as the output.
