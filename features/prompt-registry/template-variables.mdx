...
Tool variables work with both f-string and Jinja2 template formats since they operate on the tools list, not on message content.
...
For more details on setting up tool variables, see [Tool Calling - Tool Variables](/features/prompt-registry/tool-calling#tool-variables).
