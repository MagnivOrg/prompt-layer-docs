...
| `skill_collection_files_changed`     | When a Skill Collection is created, a new version is saved, a collection is deleted, or a version is restored. | <ul><li>`skill_collection_id` (string)</li><li>`source` (string: `collection_created`, `version_save`, `collection_deleted`, or `version_restored`)</li><li>`affected_paths` (array of collection-relative paths)</li><li>`version_number` (number, optional)</li></ul> |
