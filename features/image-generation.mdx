...
<Note>
The Images API uses a completion template (single text prompt in, image(s) out). If you have a chat template selected, PromptLayer will concatenate all message text into a single prompt for the API call. For best results, use a completion template.
</Note>
...
- [Run](/running-requests/promptlayer-run)
