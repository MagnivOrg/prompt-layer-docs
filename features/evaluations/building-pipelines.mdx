...
You now have a preview. Preview mode allows you to iterate with live feedback, allowing for adjustments in real-time.

## Setting up the Pipeline

### Adding Columns

Click 'Add Column' to start building your pipeline, with each column representing a step in the evaluation process.

Columns execute in order left to right. That means that if a column depends on a previous column, make sure it appears to the right of the dependency.
...
- **String Comparison**: Use this column to compare the outputs of two previous columns, showing a visual diff when relevant.
