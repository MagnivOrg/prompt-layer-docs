...
<ParamField query="created_by_email" type="string">
  Filter entities by the creator's email address. Folders, AB tests, and input variable sets only match records created after creator tracking was added for those entity types.
</ParamField>

<ParamField query="created_after" type="string">
  Filter entities created at or after this ISO 8601 datetime.
</ParamField>

<ParamField query="created_before" type="string">
  Filter entities created at or before this ISO 8601 datetime.
</ParamField>

<ParamField query="updated_after" type="string">
  Filter entities updated at or after this ISO 8601 datetime.
</ParamField>

<ParamField query="updated_before" type="string">
  Filter entities updated at or before this ISO 8601 datetime.
</ParamField>

<ParamField query="external_source" type="string">
  External ID source to filter by. Must be provided together with `external_id`.
</ParamField>

<ParamField query="external_id" type="string">
  External ID value to filter by. Must be provided together with `external_source`. If no mapping exists in the workspace, the endpoint returns an empty result set.
</ParamField>

### Sorting

<ParamField query="sort_by" type="string">
  Sort by `created_at`, `updated_at`, `name`, or `id`.
</ParamField>
...
<ResponseField name="entities" type="Entity[]">
  <Expandable title="Entity object">
    ...
    <ResponseField name="external_ids" type="ExternalId[]">External ID mappings attached to the entity.</ResponseField>
  </Expandable>
</ResponseField>
...
- **Explicit sort**: When `sort_by` is provided and semantic/text search ordering does not take precedence, results are sorted by the requested field and `sort_order`.
...
```json
{
  "entities": [
    ...
    },
    {
      ...
      "external_ids": [
        { "source": "acme_cms", "external_id": "prompt_205" }
      ]
    },
    {
      "id": 88,
