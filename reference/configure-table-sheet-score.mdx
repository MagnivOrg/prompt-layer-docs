...
For custom scoring, pass `code` and optionally `code_language` (`PYTHON` by default, or `JAVASCRIPT`). Custom code receives a `data` list of row dicts keyed by column title. See [Tables Data Model](/features/tables/data-model#custom-score-and-code-execution-input-data) for per-column value shapes.

Boolean scoring also supports `true_values`, `false_values`, and `assertion_aggregation` (`all`, `any`, or `mean`).
