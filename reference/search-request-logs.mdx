...
Use `filter_group` to narrow results by any request log field. Wrap your filters in a group with `"logic": "AND"` or `"logic": "OR"` to combine them.
...
<Info>
**Understanding nested fields:** The `output`, `metadata`, and `input_variables` fields are flattened from nested objects into searchable key-value pairs using dot-notation (e.g. `result.status`). See the [Search Data Model](/features/prompt-history/search-data-model) guide for details on how different output types are indexed and when to use `key_equals` vs `key_contains`.
</Info>

<Warning>
**Input variables are only indexed if they are referenced in your prompt template.** Unused input variables will not appear in search results. If you need to filter by values not in the template, attach them as [metadata](/features/prompt-history/metadata) instead.
</Warning>
...
Filter groups can be nested recursively to build complex AND/OR queries.
...
- [Search Suggestions](/reference/search-request-suggestions) - Get autocomplete suggestions for search fields
- [Search Data Model](/features/prompt-history/search-data-model) - How your data is indexed and what's searchable
